...
code() {
    ...
    [ $run_weak -eq 1 ] && "$SCRIPT_DIR"/coding.sh "openai/$WEAK_MODEL" "$1" "$FILES" || { [ $run_weak -eq 1 ] && run_weak=0 && git reset --hard;  "$SCRIPT_DIR"/coding.sh "openai/$STRONG_MODEL" "$1" "$FILES" ; } || { echo "Failed to implement request. Quitting." && git reset --hard && exit 1; } && cargo fmt
}
